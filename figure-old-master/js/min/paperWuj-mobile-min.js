var container = new Shape.Rectangle(new Point(0, 0), [view.bounds.width,120])
...
for (var i = 1; i <= totalPoints3; i++) {
  movingDot3 = "point" + i
  pointXPos3 = Math.floor(Math.random() * container.bounds.width);
  pointYPos3 = Math.floor(Math.random() * container.bounds.height);
...
for (var i = 1; i <= totalPoints4; i++) {
  movingDot4 = "point" + i
  pointXPos4 = Math.floor(Math.random() * container.bounds.width);
  pointYPos4 = Math.floor(Math.random() * container.bounds.height);
  var position4 = new Point (pointXPos4, pointYPos4)
  movingDot4 = new Shape.Circle (position4,1)
  movingDot4.fillColor = "none"
  movingDot4.opacity = 0
  position4.data = {
    xDirection4: 'right',
    yDirection4: "up"
  }
  polygonPoints4.push(position4);

}

var myPath4 = new Path();
var pathSegments4 = myPath4.segments
//red
myPath4.fillColor = "#ffcc00"
myPath4.blendMode = 'multiply'


myPath4.closed = true;
for (var i = 0; i <= polygonPoints4.length - 1; i++) {
  myPath4.add(polygonPoints4[i]);
}
...
var clipCircle = new Shape.Rectangle(new Point(0, 0), [view.bounds.width,120])
...
function onFrame(event) {
    document.getElementById("canvasMobile").style.backgroundColor = "#111";
  ...
  for (var i = 0; i <= totalPoints - 1; i++) {
    ...
    var item = animationLayer1.children[i]
        ...
        if (item.data.xDirection === "right"){
          item.position.x += item.bounds.width/3;
        } else {
          item.position.x -= item.bounds.width/3;
        }
    ...
    myPath.smooth()
        ...
        if (item2.bounds.left < minX){
          item2.data.xDirection = "right"
            ...
            if (item2.bounds.top < minY - 20){
              item2.data.yDirection = "down"
            }
            
            if (item2.bounds.bottom > maxY + 20){
              item2.data.yDirection = "up"
            }
            
            // move item
            if (item2.data.xDirection === "right"){
          item2.position.x += item2.bounds.width/3;
        } else {
          item2.position.x -= item2.bounds.width/3;
        }
            
            if (item2.data.yDirection === "up"){
              item2.position.y = sinus2 * maxY
            } else {
              item2.position.y = sinus2 * maxY
            }
        ...
        for (var i = 0; i <= totalPoints3 - 1; i++) {
          var sinus3 = Math.cos(event.time * 0.5 + i)
          var atEnd3
          var atStart3
          var item3 = animationLayer3.children[i]
            ...
            if (item3.data.xDirection === "right"){
              item3.position.x += item3.bounds.width / 10;
            } else {
              item3.position.x -= item3.bounds.width / 10;
            }
            
            if (item3.data.yDirection === "up"){
              item3.position.y = sinus3 * maxY
            } else {
              item3.position.y = sinus3 * maxY
            }
        ...
        myPath3.smooth()
                ...
                // 4nd path (blue)
        for (var i = 0; i <= totalPoints4 - 2; i++) {
          var sinus4 = Math.sin(event.time * 0.3 + i)
          var atEnd4
          var atStart4
          var item4 = animationLayer4.children[i]
            //add positions to array
            positions4.push(item4.position)
            // check directions
           if (item4.bounds.right > maxX){
          item4.data.xDirection = "left"
        }
        if (item4.bounds.left < minX){
          item4.data.xDirection = "right"
        }
            
            if (item4.bounds.top < minY - 20){
              item4.data.yDirection = "down"
            }
            
            if (item4.bounds.bottom > maxY + 20){
              item4.data.yDirection = "up"
            }
            
            // move item
            if (item4.data.xDirection === "right"){
          item4.position.x += item4.bounds.width/3;
        } else {
          item4.position.x -= item4.bounds.width/3;
        }
            
            if (item4.data.yDirection === "up"){
              item4.position.y = sinus4 * maxY
            } else {
              item4.position.y = sinus4 * maxY
            }
        ...
        for (var key in myPath4.segments) {
          if (myPath4.segments.hasOwnProperty(key)) {
            myPath4.segments[key].point = positions4[key]
          }
        }
        myPath4.smooth()
